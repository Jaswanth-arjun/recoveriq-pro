...
import type { Category, WorldKind } from "@/data/categories";
...
function Stalks({ count, color, seed }: { count: number; color: string; seed: number }) {
  const mesh = useRef<THREE.InstancedMesh>(null);
  const items = useMemo(() => {
    const r = rand(seed);
    return Array.from({ length: count }, () => ({
...
export function Particles({
  ...
  useFrame(({ clock }) => {
    ...
    const t = clock.elapsedTime;
    items.forEach((it, i) => {
      const y = ((it.y + t * rise * it.s) % 9) - 0.5;
      dummy.position.set(it.x + Math.sin(t * 0.4 + it.p) * 0.7, y, it.z);
      dummy.scale.setScalar(size * it.s);
      dummy.updateMatrix();
      mesh.current!.setMatrixAt(i, dummy.matrix);
    });
...
const WORLDS: Record<WorldKind, () => ReactElement> = {
  pasture: Pasture,
  bakery: BakeryWorld,
  field: FieldWorld,
...
export function World({
  ...
  const Body = WORLDS[category.world];
  return (
    <group ref={group}>
      <Body />
      <Particles
