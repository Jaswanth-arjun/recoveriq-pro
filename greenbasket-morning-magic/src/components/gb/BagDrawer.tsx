...
export function BagDrawer({ onContinue }: { onContinue: () => void }) {
    ...
    <>
      ...
      >
        <header className="flex shrink-0 items-start justify-between border-b border-white/45 px-6 py-5">
          <div>
            <h2 className="display-title text-3xl text-ink">My morning bag</h2>
          ...
          <button
            ...
            className="grid size-9 place-items-center rounded-full bg-white/60 text-ink transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf"
          >
            <X className="size-4" />
          </button>
        </header>
          ...
          {lines.map(({ product, quantity, daily: d }) => (
            ...
            >
              <div
                ...
                style={{
                  background: `radial-gradient(120% 120% at 30% 20%, #fff 0%, ${product.swatch} 70%)`,
                }}
                aria-hidden
              >
                {product.image}
        ...
        <footer className="shrink-0 space-y-3 border-t border-white/45 px-6 py-5">
          <div className="flex items-baseline justify-between">
            ...
            <span className="display-title tabular-nums text-2xl text-leaf-deep">
              {formatINR(daily)}
            </span>
          </div>
          <div className="flex items-baseline justify-between">
            ...
            <span className="display-title tabular-nums text-3xl text-ink">
              {formatINR(monthly)}
            </span>
          </div>
          <button
            ...
            className="w-full rounded-full bg-leaf-deep py-3 text-xs font-medium uppercase tracking-[0.28em] text-cream transition hover:bg-leaf focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf focus-visible:ring-offset-2"
          >
            Continue
          </button>
        </footer>
