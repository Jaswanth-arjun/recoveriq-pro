...
/**
 * Controlled, snapping section navigation.
 * Scrolling with the pointer inside an element marked `data-inner-scroll`
 * scrolls that element natively until it hits an edge; only then does the
 * outer world advance smoothly one page at a time without skipping.
 */
export function useSectionScroll(total: number) {
  ...
  useEffect(() => {
    /** returns true when the inner panel consumed the gesture */
    const innerConsumes = (target: EventTarget | null, delta: number) => {
      ...
      // at an edge: dwell briefly before letting the world move on
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if ((e.target as HTMLElement)?.closest?.("[data-scroll-ignore]")) return;
